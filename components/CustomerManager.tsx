...
const CustomerManager: React.FC<Props> = ({ customers, orders, shippingNotes, payments, onAdd, onUpdate, onAddPayment, onDeletePayment, onReorderOrders, user }) => {
   ...
   // Tự động cập nhật selectedCustomer khi customers props thay đổi
   useEffect(() => {
      if (selectedCustomer) {
         const freshCustomer = customers.find(c => c.id === selectedCustomer.id);
         if (freshCustomer && JSON.stringify(freshCustomer) !== JSON.stringify(selectedCustomer)) {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      if (editingCustomer) {
         const updatedCustomer = { ...editingCustomer, ...newCustomer } as Customer;
         try {
                                 ...
                                 <tr key={note.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-5">
                                       <div className="flex flex-col items-center">
                                          ...
                                          <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase mt-1 ${note.status === 'overdue' ? 'bg-rose-600 text-white' :
